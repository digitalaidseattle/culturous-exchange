...
class CEPlanService extends EntityService<Plan> {
  ...
  async duplicate(plan: Plan): Promise<Plan> {
    ...
    return this.insert(proposed).then(async (duplicatePlan) => {
      console.log("duplicatePlan", duplicatePlan);
      const duplicatePlacements = plan.placements.map((placement) => {
        return {
          ...placement,
          plan_id: duplicatePlan.id,
        };
      });
      const duplicateGroups = plan.groups.map((group) => {
        return {
          ...group,
          plan_id: duplicatePlan.id,
        };
